...
/**
 * ============================================================================
 * Redis Bot Protection - SEO ОПТИМІЗОВАНА ВЕРСІЯ v3.8.2 (UNIFIED WHITELIST)
 * ============================================================================
 * 
 * ВЕРСІЯ 3.8.2 - UNIFIED IP WHITELIST (2026-01-26)
 * 
 * НОВЕ v3.8.2:
 * 🔥 Єдина функція _is_whitelisted_ip() для перевірки ВСІХ білих списків
 * 🔥 ВСІ IP з будь-якого білого списку пропускають ВСІ перевірки однаково
 * 🔥 Спрощена логіка: один виклик замість двох окремих перевірок
 * 
 * БІЛІ СПИСКИ IP (обидва працюють ОДНАКОВО):
 * - $ADMIN_IP_WHITELIST    - власні/адмінські IP
 * - $SEARCH_ENGINE_IP_RANGES - IP пошукових систем
 * 
 * ЩО ПРОПУСКАЄТЬСЯ для IP з білих списків:
 * ✅ JS Challenge (PoW)
 * ✅ Rate Limit
 * ✅ Burst Detection  
 * ✅ UA Rotation
 * ✅ No-Cookie Attack Detection
 * 
 * ВЕРСІЯ 3.8.1 - ADMIN IP WHITELIST + STATS FIX (2026-01-26)
 * 
 * НОВЕ v3.8.1:
 * 🔥 Білий список адмінських/власних IP ($ADMIN_IP_WHITELIST)
 * 🔥 Адмінські IP пропускають ВСІ перевірки (JS Challenge, Rate Limit, і т.д.)
 * 🔥 Підтримка IPv4 та IPv6 з CIDR нотацією
 ...
 * 
 * НАЛАШТУВАННЯ АДМІНСЬКИХ IP:
 * $ADMIN_IP_WHITELIST = array(
 *     '212.84.160.58/32',           // Одиночний IPv4
 *     '212.84.160.58',              // Те саме без /32
 *     '10.0.0.0/8',                 // Підмережа
...
// ============================================================================
// БІЛИЙ СПИСОК АДМІНСЬКИХ/ВЛАСНИХ IP (v3.8.1) - НАЙВИЩИЙ ПРІОРИТЕТ!
// ============================================================================
/**
 * IP адреси які ЗАВЖДИ пропускаються БЕЗ будь-яких перевірок!
 * 
 * ⚠️ УВАГА: Ці IP повністю обходять захист! Використовуй тільки для:
 * - Власних IP адрес (дім, офіс)
 * - IP серверів моніторингу
 * - Довірених партнерів
 * 
 * Формати:
 * - Одиночний IP: '212.84.160.58/32' або '212.84.160.58'
 * - Підмережа: '192.168.1.0/24'
