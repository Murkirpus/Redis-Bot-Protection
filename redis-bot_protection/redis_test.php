...
// Функции для работы с Redis-системой защиты v2.0
...
<head>
   ...
   <title>🛡️ Redis MurKir Security Test v2.0 - User Hash Protection</title>
   <style>
       body {
...
<body>
   <div class="container">
       <div class="header">
           <div class="version-badge">🛡️ Bot Protection v2.0</div>
       ...
       <div class="status-card redis <?php echo !$protectionActive ? 'error' : ''; ?>">
           ...
           <?php if ($protectionActive): ?>
               ...
               <?php if ($redisStats): ?>
                   <div class="metrics">
                       ...
                       <div class="metric">
                           <div class="number"><?php echo $redisStats['blocked_sessions'] ?? 0; ?></div>
           ...
           <?php else: ?>
               ...
               <div class="highlight-box">
                   ...
                   <ul>
                       <li>Проверьте, запущен ли Redis сервер</li>
                       <li>Убедитесь в правильности настроек подключения</li>
                       <li>Проверьте права доступа к Redis</li>
                   </ul>
               </div>
           <?php endif; ?>
       </div>
           ...
           <?php if ($isVerified): ?>
               ...
               <?php if ($trustScore > 70): ?>
                   <div style="color: #28a745; font-weight: bold; margin-top: 15px; padding: 10px; background: rgba(40, 167, 69, 0.1); border-radius: 8px;">
                       🌟 VIP пользователь - высокий уровень доверия!
                   </div>
           ...
           <div class="info-grid">
               <div>
                   ...
                   <div class="hash-display" onclick="copyToClipboard(this)" title="Нажмите для копирования">
                       <?php echo htmlspecialchars(substr($userHashInfo['user_hash'], 0, 64)); ?>...
                   </div>
               ...
               <div>
                   ...
                   <table class="table">
                       ...
                       <tr>
                           <td><strong>Сессий:</strong></td>
                           <td><?php echo count(array_unique($userHashInfo['tracking_data']['session_ids'] ?? [])); ?></td>
                       </tr>
                   </table>
               </div>
               <?php endif; ?>
           ...
           <div class="info-grid">
               <div>
                   ...
                   <table class="table">
                       ...
                       <tr>
                           <td><strong>Сессионный хеш:</strong></td>
                           <td><span class="redis-key"><?php echo $userHashDiagnosis['session_hash']; ?></span></td>
                       </tr>
                       <tr>
           ...
           <div class="info-box">
               ...
               <table class="table">
                   ...
                   <tr>
                       ...
                       <td><span class="redis-key" onclick="copyToClipboard(this)"><?php echo substr(session_id(), 0, 20); ?>...</span></td>
                   </tr>
                   <tr>
       ...
       <div id="session-info" class="tab-content">
           <div class="info-box">
               ...
               <?php else: ?>
                   <div class="highlight-box">
                       <p style="color: #856404; margin: 0;">⚠️ Данные сессии недоступны</p>
                       <p style="margin: 10px 0 0 0; font-size: 0.9em;">Возможно, система защиты не инициализирована или произошла ошибка.</p>
                   </div>
               <?php endif; ?>
               
               <h4 style="margin-top: 30px;">📋 Все данные $_SESSION:</h4>
       ...
       <div id="redis-keys" class="tab-content">
           <div class="info-box">
               ...
               <h4>📋 Структура Redis ключей v2.0:</h4>
               <table class="table">
                   <thead>
                       <tr>
                           <th>Тип ключа</th>
                   ...
                   <tbody>
                       ...
                       <tr>
                           <td>Session Data</td>
                           <td><code>bot_protection:session:data:</code></td>
                           <td><span class="redis-key"><?php echo substr(session_id(), 0, 12); ?>...</span></td>
                           <td>Данные сессии</td>
                           <td><span class="security-badge security-safe">Active</span></td>
                       </tr>
                       <tr>
                           <td>Session Block</td>
                           <td><code>bot_protection:session:blocked:</code></td>
                           <td><span class="redis-key"><?php echo substr(session_id(), 0, 12); ?>...</span></td>
       ...
       <div id="user-hash-stats" class="tab-content">
           <div class="info-box new-feature">
               ...
               <?php if ($userHashStats): ?>
                   ...
                   <div class="highlight-box">
                       <h4>💡 Что такое хеш пользователя?</h4>
                       <p>Хеш пользователя - это уникальный идентификатор, создаваемый на основе:</p>
                       <ul>
                           <li>🌐 User-Agent браузера</li>
                           <li>🗣️ Языковых настроек</li>
                           <li>⚙️ Настроек кодировки</li>
                           <li>📱 Типа устройства (мобильное/десктоп)</li>
						   <li>🔗 Части IP-адреса (для стабильности)</li>
                           <li>🔐 Секретного ключа</li>
                       </ul>
                       <p><strong>Преимущества:</strong> Блокировка работает даже при смене IP, очистке cookies или новых сессиях.</p>
                   </div>
               <?php else: ?>
                   <div class="highlight-box">
       ...
       <div id="ttl-settings" class="tab-content">
           <div class="info-box">
               <h3>⏱️ TTL настройки системы v2.0</h3>
               <?php if ($ttlSettings): ?>
                   <div class="highlight-box">
                           ...
                           <?php foreach ($ttlSettings as $key => $value): ?>
                               <tr <?php if (strpos($key, 'user_hash') !== false) echo 'class="new-feature" style="background: rgba(156, 39, 176, 0.05);"'; ?>>
                                   ...
                                   <td>
                                       <?php
                                       $descriptions = [
                                           'tracking_ip' => 'Отслеживание IP активности (сокращено с 24ч)',
                                           'session_data' => 'Данные сессии пользователя (сокращено с 24ч)',
                                           'session_blocked' => 'Блокировка сессии (сокращено с 24ч)',
                                           'cookie_blocked' => 'Блокировка cookie (сокращено с 24ч)',
                                           'ip_blocked' => 'Базовая блокировка IP (сокращено)',
                                           'ip_blocked_repeat' => 'Блокировка повторных нарушителей',
                                           'rdns_cache' => 'Кеш rDNS запросов (сокращено)',
                                           'logs' => 'Хранение логов (сокращено с 7 дней)',
                   ...
                   <div style="margin-top: 20px; padding: 15px; background: rgba(40, 167, 69, 0.1); border-radius: 8px; border-left: 4px solid #28a745;">
                       <h5 style="margin-top: 0; color: #155724;">🎯 Преимущества оптимизации v2.0:</h5>
                       <ul style="margin-bottom: 0; color: #155724;">
                           <li>⚡ Улучшенная производительность Redis</li>
                           <li>💾 Сниженное потребление памяти</li>
                           <li>🧹 Автоматическая очистка каждые 30 минут</li>
                           <li>🔐 Новая система хеш-блокировки для устойчивости</li>
...
redis-cli keys "bot_protection:user_hash:*" | xargs redis-cli del
       ...
       <div id="debug" class="tab-content">
           <div class="info-box">
               ...
               <h4>🛡️ Browser Security Analysis:</h4>
               <pre><?php 
               $securityAnalysis = [
       ...
       <div class="info-box" style="border-left-color: #dc3545;">
           ...
           <div style="margin: 20px 0;">
               ...
               <h4>🔧 Навигация:</h4>
               <a href="redis_test.php" class="btn">👁️ Обычный режим</a>
               <?php if (file_exists('redis-admin.php')): ?>
       ...
       <div style="text-align: center; color: #6c757d; font-size: 0.9em;">
           <div style="margin-bottom: 10px;">
               🛡️ <strong>Redis MurKir Security System v2.0</strong> with User Hash Support
           </div>
           <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 15px; font-size: 0.85em;">
               ...
               <span>🔐 Session: <?php echo substr(session_id(), 0, 8); ?>...</span>
   ...
   <script>
       ...
       window.botProtectionTest = {
           ...
           simulateBot: function() {
               console.log('🤖 Simulating bot behavior v2.0...');
           ...
           simulateHuman: function() {
               console.log('👤 Simulating human behavior v2.0...');
           ...
           testUserHash: function() {
               console.log('🔐 Testing user hash v2.0...');
           ...
           performanceTest: function() {
               console.log('🚀 Starting performance test v2.0...');
           ...
           analyzeUserHash: function() {
               ...
               console.log('🔍 Advanced browser fingerprint analysis v2.0:', fingerprint);
           ...
           getCanvasFingerprint: function() {
               try {
                   ...
                   ctx.font = '14px Arial';
                   ctx.fillText('BotProtection v2.0 🛡️', 2, 2);
           ...
           stressTest: function() {
               console.log('💥 Starting stress test v2.0...');
       ...
       setInterval(() => {
           ...
           console.log('👤 User Activity Analysis v2.0:', activityData);
       ...
       document.addEventListener('keydown', (e) => {
           if (e.ctrlKey || e.metaKey) {
               switch(e.key) {
                   ...
                   case '2': e.preventDefault(); showTab('session-info'); break;
       ...
       document.addEventListener('DOMContentLoaded', () => {
           ...
           setTimeout(() => {
               ...
               botProtectionTest.showNotification('🛡️ Bot Protection v2.0 с максимальной защитой активна!', 'success');
               <?php else: ?>
           ...
           setTimeout(() => {
               botProtectionTest.showNotification('💡 Горячие клавиши: Ctrl+1-7 (табы), Ctrl+B (бот), Ctrl+H (человек), Ctrl+P (производительность)', 'info');
           }, 4000);
       });
...
Trust Score: <?php echo $trustScore; ?>%
       ...
       console.log('🛡️ Bot Protection Test Page v2.0 fully loaded and initialized');
