...
class RedisBotProtectionWithSessions {
   ...
   private function generateUserHash($ip = null) {
       ...
       // ИСПРАВЛЕНИЕ: Добавляем уникальность для каждой вкладки/сессии
       ...
       // НОВОЕ: Добавляем временную метку и случайность для инкогнито
       $timestamp = microtime(true);
       $randomComponent = mt_rand(1000, 9999);
       
       // Проверяем признаки инкогнито режима
       $isIncognito = $this->detectIncognitoMode();
       
       if ($isIncognito) {
           // Для инкогнито добавляем больше уникальности
           $uniqueId = uniqid('incognito_', true) . '_' . $randomComponent . '_' . $timestamp;
           error_log("Incognito mode detected, adding unique ID: " . substr($uniqueId, 0, 20) . "...");
       } else {
           $uniqueId = $sessionId;
       }
       
       if ($this->isMobileDevice($userAgent)) {
           // Для мобильных добавляем часть IP + уникальный ID
           $ipPart = $this->getIPFingerprint($ip);
           $fingerprint .= '|mobile|' . $ipPart . '|' . $uniqueId;
           error_log("Mobile device hash: Browser={$browserInfo['name']}, IP_part=$ipPart, UniqueID=" . substr($uniqueId, 0, 8) . "...");
       } else {
           // Для десктопа используем полный IP + уникальный ID
           $fingerprint .= '|desktop|' . $ip . '|' . $uniqueId;
           error_log("Desktop device hash: Browser={$browserInfo['name']}, Full_IP=$ip, UniqueID=" . substr($uniqueId, 0, 8) . "...");
       }
   ...
   /**
    * НОВАЯ функция определения инкогнито режима
    */
   private function detectIncognitoMode() {
       // Признаки инкогнито режима:
       
       // 1. Отсутствие cookies при первом запросе
       $hasCookies = !empty($_COOKIE);
       
       // 2. Отсутствие Referer при переходе на сайт
       $hasReferer = !empty($_SERVER['HTTP_REFERER']);
       
       // 3. Новая сессия без предыдущих данных
       $sessionId = session_id();
       $isNewSession = empty($_SESSION) || empty($sessionId);
       
       // 4. Проверяем наличие типичных cookies
       $hasTrackingCookies = isset($_COOKIE[$this->cookieName]) || 
                            isset($_COOKIE['PHPSESSID']) ||
                            count($_COOKIE) > 2;
       
       // Эвристика для определения инкогнито
       $incognitoScore = 0;
       
       if (!$hasCookies) $incognitoScore += 3;
       if (!$hasReferer && $_SERVER['REQUEST_URI'] !== '/') $incognitoScore += 2;
       if ($isNewSession) $incognitoScore += 2;
       if (!$hasTrackingCookies) $incognitoScore += 1;
       
       $isIncognito = $incognitoScore >= 4;
       
       if ($isIncognito) {
           error_log("Incognito mode detected: Score=$incognitoScore, Cookies=" . count($_COOKIE) . 
                    ", Session=" . ($sessionId ?: 'none') . ", Referer=" . ($_SERVER['HTTP_REFERER'] ?? 'none'));
       }
   ...
   private function analyzeUserHashBehavior() {
       ...
       if (!$trackingData || $trackingData['requests'] < 2) { // Снизили с 3 до 2
       ...
       // Сбалансированные пороги - теперь легче достичь
       $blockThreshold = $isMobile ? 8 : 6; // Было 12/10 - еще снизили
