...
class RedisBotProtectionWithSessions {
  ...
  private function trackUserHashActivity() {
      ...
      if ($existing) {
          ...
          if (count($existing['request_times']) > 50) {
              $existing['request_times'] = array_slice($existing['request_times'], -50);
          }
          if (count($existing['pages']) > 100) {
              $existing['pages'] = array_unique(array_slice($existing['pages'], -100));
          }
          if (count($existing['ips']) > 20) {
              $existing['ips'] = array_unique(array_slice($existing['ips'], -20));
          }
   ...
   * Помогает понять, какой хеш генерируется для текущего пользователя
   */
